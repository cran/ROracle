  ...
  for (conID = 0; conID < pctx->max_roociCtx; conID++)
    if (pctx->con_roociCtx[conID] &&
        !rodbiAssertCon((pctx->con_roociCtx[conID])->parent_roociCon, 
                        __FUNCTION__, 1))
    {   
      roociTerminateCon(pctx, pctx->con_roociCtx[conID], FALSE);
  ...
  for (resID = 0; resID < pcon->max_roociCon; resID++)
  {
    if (pcon->res_roociCon[resID] &&
        !rodbiAssertRes((pcon->res_roociCon[resID])->parent_roociRes,
                         __FUNCTION__, 1))
    { 
      roociResFree(pcon, pcon->res_roociCon[resID]);
...
/* ---------------------------- roociAllocateTZdescriptors ---------------- */

static sword roociAllocateTZdescriptors(roociCtx *pctx, roociRes *pres,
                                        roociCon *pcon)
{
  sword          rc = OCI_SUCCESS;

  /* allocate POSIXct beginning datetime descriptor and interval descriptor */
  if ((!pres->epoch_roociRes) && (!pres->diff_roociRes))
  {
    rc = OCIDescriptorAlloc(pctx->env_roociCtx,
                            (void **)&(pres->epoch_roociRes),
                            OCI_DTYPE_TIMESTAMP_TZ, 0, NULL);
    if (rc == OCI_ERROR)
      return rc;

    rc = OCIDescriptorAlloc(pctx->env_roociCtx,
                            (void **)&(pres->diff_roociRes),
                            OCI_DTYPE_INTERVAL_DS, 0, NULL);
    if (rc == OCI_ERROR) 
      return rc;
  
    /* construct the beginning of 1970 (in the UTC timezone) */
    rc = OCIDateTimeConstruct(pctx->env_roociCtx, pcon->err_roociCon,
                              pres->epoch_roociRes,
                              1970, 1, 1, 0, 0, 0, 0, (OraText*)"UTC", 3);
    if (rc == OCI_ERROR)
      return rc;
  }

  /*
  ** allocate an interval for time difference between UTC and session's
  ** timezone - this is required for sending and receiving OCIDateTime
  ** values
  */
  if (!pcon->tzdiff_roociCon)
  {
    sb1          tzhr;
    sb1          tzmm;
    OCIDateTime *systime;
    ...
    rc = OCIDescriptorAlloc(pctx->env_roociCtx,
                            (void **)&(pcon->tzdiff_roociCon),
                            OCI_DTYPE_INTERVAL_DS, 0, NULL);
    if (rc == OCI_ERROR)
      return rc;

    rc = OCIDescriptorAlloc(pctx->env_roociCtx, (void **)&systime,
                           OCI_DTYPE_TIMESTAMP_TZ, 0, NULL);
    if (rc == OCI_ERROR)
      return rc;

    /* get the current datetime containing the OCI session's timezone */
    if (pcon->timesten_rociCon)
    {
      /* TimesTen requires the environment handle rather than the session */
      /* due lack of time zone support */
      rc = OCIDateTimeSysTimeStamp(pctx->env_roociCtx, pcon->err_roociCon,
                                   systime);
    }
    else
      rc = OCIDateTimeSysTimeStamp(pcon->usr_roociCon, pcon->err_roociCon,
                                   systime);
    if (rc == OCI_ERROR)
    {
    ...
    /* get the offsets for the OCI session's timezone */
    rc = OCIDateTimeGetTimeZoneOffset(pcon->usr_roociCon, pcon->err_roociCon,
                                      systime, &tzhr, &tzmm);
    OCIDescriptorFree (systime, OCI_DTYPE_TIMESTAMP_TZ);
    if (rc == OCI_ERROR)
      return rc;
    ...
    /* create the interval */
    rc = OCIIntervalSetDaySecond(pcon->usr_roociCon, pcon->err_roociCon,
                                 0, (sb4)tzhr, (sb4)tzmm, 0, 0,
                                 pcon->tzdiff_roociCon);
    if (rc == OCI_ERROR)
     return rc;
  }

  return rc;
} /* end of roociAllocateTZdescriptors */
                         ...
                         ub4 stmt_cache_siz, ub4 lob_prefetch_siz)
{
  ...
  rc = OCIHandleAlloc(pctx->env_roociCtx, (void **)&pcon->err_roociCon,
                      OCI_HTYPE_ERROR, (size_t)0, (void **)NULL);
  if (rc == OCI_ERROR)
    return rc;

  /* allocate authentication handle */
  rc = OCIHandleAlloc(pctx->env_roociCtx, (void **)&pcon->auth_roociCon, 
  ...
  /* enable LOB prefetching by setting lob prefetch size */
  rc = OCIAttrSet (pcon->usr_roociCon, OCI_HTYPE_SESSION, 
                   (void *)&lob_prefetch_siz, 0, 
                   OCI_ATTR_DEFAULT_LOBPREFETCH_SIZE, pcon->err_roociCon);
  if (rc == OCI_ERROR)
    return ROOCI_DRV_ERR_CON_FAIL;
...
sword roociGetError(roociCtx *pctx, roociCon *pcon, sb4 *errNum, text *errMsg)
{
  ...
  else if (pctx->env_roociCtx)             /* if environment handle defined */
  {
...
sword roociExecTTOpt(roociCon *pcon)
{
  ...
  rc = OCIHandleAlloc((void *)pcon->ctx_roociCon->env_roociCtx,
                      (void **)&stmthp, OCI_HTYPE_STMT, 0, 0);
  if (rc != OCI_SUCCESS)
    return rc;
...
sword roociTerminateCtx(roociCtx *pctx)
{
  ...
  if (pctx->con_roociCtx)
  {
    for(conID = 0; conID < pctx->max_roociCtx; conID++)
    {
      if (pctx->con_roociCtx[conID])
      {
        rc = roociTerminateCon(pctx, pctx->con_roociCtx[conID], TRUE);
        if (rc == OCI_ERROR)
          return rc;
      }
    }
...
sword roociTerminateCon(roociCtx *pctx, roociCon *pcon, boolean validCon)
{
  ...
  if (pcon->res_roociCon)
  {
    ...
    {    
      if (pcon->res_roociCon[resID])
      {    
        rc = roociResFree(pcon, pcon->res_roociCon[resID]);
        if (rc == OCI_ERROR)
          return rc;
      }    
    }    
  ...
  /* free tzdiff interval descriptor */
  if (pcon->tzdiff_roociCon)
  {
    rc = OCIDescriptorFree(pcon->tzdiff_roociCon, OCI_DTYPE_INTERVAL_DS);
    if (rc == OCI_ERROR)
      return rc;
  }
...
sword roociStmtExec(roociCon *pcon, roociRes *pres, ub4 noOfRows, ub2 styp,
                   int *rows_affected) 
{
  sword         rc              = OCI_ERROR;
  roociThrCtx   thrCtx;
                    ...
                    ub1 form_of_use)
{
  ...
  rc = OCIBindByPos(pres->stm_roociRes, &bndp, pcon->err_roociCon, bufPos,
                    ...
                    pres->alen_roociRes[bufPos-1], NULL, (ub4)0, NULL,
                    OCI_DEFAULT);
  if (rc == OCI_ERROR)
    return rc;
  else
    rc = OCIBindArrayOfStruct(bndp, pcon->err_roociCon,
                              pres->bsiz_roociRes[bufPos-1],
                              sizeof(sb2), sizeof(ub2), 0);
                     ...
                     ub4 lobPrefetchSize)
{
  ...
  size_t          nrows;
  OCIDateTime   **tsdt;
  boolean         lobPrefetchLenSet = FALSE;    /* LOB prefetch length flag */
  ...
  for (cid = 0; cid < pres->ncol_roociRes; cid++)
  {
    ...
    {
      tsdt = (OCIDateTime **)(pres->dat_roociRes[cid]);
      ...
      if (pcon->timesten_rociCon)
      {
        ...
        rc = OCIArrayDescriptorAlloc(pctx->env_roociCtx, (void **)tsdt,
                                     OCI_DTYPE_TIMESTAMP_TZ, nrows, 0, NULL);
      if (rc == OCI_ERROR)
        return rc;
    }
    ...
    if ((etyp == SQLT_CLOB) ||
        ...
        (etyp == SQLT_BFILE))
    {
      /* Override default LOB prefetch size */
      rc = OCIAttrSet(defp, OCI_HTYPE_DEFINE, (void *)&lobPrefetchSize, 0, 
                      OCI_ATTR_LOBPREFETCH_SIZE, pcon->err_roociCon);
      if (rc == OCI_ERROR)
        return rc;
  
      /* Set lob prefetch length if lob prefetch is enabled */
      if (lobPrefetchSize > 0)
      {
        /* set LOB prefetch length attribute */
        lobPrefetchLenSet = TRUE;
        rc = OCIAttrSet (defp, OCI_HTYPE_DEFINE, (void *)&lobPrefetchLenSet, 0,
            OCI_ATTR_LOBPREFETCH_LENGTH, pcon->err_roociCon); 
        if (rc == OCI_ERROR)
          return rc;
      }
    }
                            ...
                            ub4 colId, ub4 *len, oratext **buf)
{
  ...
  rc = OCIParamGet(pres->stm_roociRes, OCI_HTYPE_STMT, 
                   pcon->err_roociCon, (void **)&par, colId);
  if (rc == OCI_ERROR)
  {
...
sword roociFetchData(roociCon *pcon, roociRes *pres, ub4 *rows_fetched,
...
sword roociReadLOBData(roociRes *pres, int *lob_len, int rowpos, int cid)
{
  ...
  lob_loc = *(OCILobLocator **)(pres->dat_roociRes[cid] +
                            ...
                            boolean isDateCol)
{
  ...
  /* allocate POSIXct beginning datetime descriptor and interval descriptor */
  rc = roociAllocateTZdescriptors(pctx, pres, pcon);
  if (rc == OCI_ERROR)
    return rc;

  /* compute the difference from the epoch */
  rc = OCIDateTimeSubtract(pctx->env_roociCtx, pcon->err_roociCon,
                           tstm, pres->epoch_roociRes, pres->diff_roociRes);
...
sword roociWriteDateTimeData(roociRes *pres, OCIDateTime *tstm, double date)
{
  ...
  rc = OCIDateTimeIntervalAdd(pctx->env_roociCtx, pcon->err_roociCon,
                              pres->epoch_roociRes, pres->diff_roociRes, tstm);
  if (rc == OCI_ERROR)
    return rc;
...
sword roociResFree(roociCon *pcon, roociRes *pres)
{
  ...
  if (pres->bdat_roociRes)
  {
    for (bid = 0; bid < pres->bcnt_roociRes; bid++)
      ...
      {    
        ...
        {
          void **tsdt = (void **)(pres->bdat_roociRes[bid]);
    ...
    for (cid = 0; cid < pres->ncol_roociRes; cid++)
      if (pres->dat_roociRes[cid])
      {
        ...
        if ((exttype == SQLT_CLOB) ||
            ...
            (exttype == SQLT_BFILE))
        {
          OCILobLocator **lob = (OCILobLocator **)(pres->dat_roociRes[cid]);
          rc = OCIArrayDescriptorFree((void **)lob,
                                      (exttype == SQLT_BFILE) ?
                                                               OCI_DTYPE_FILE :
                                                               OCI_DTYPE_LOB);
          if (rc == OCI_ERROR)
  ...
  /* free epoch time descriptor */
  if (pres->epoch_roociRes)
  {
    rc = OCIDescriptorFree(pres->epoch_roociRes, OCI_DTYPE_TIMESTAMP_TZ);
    if (rc == OCI_ERROR)
      return rc;
  }

  /* free diff interval descriptor */
  if (pres->diff_roociRes)
  {
    rc = OCIDescriptorFree(pres->diff_roociRes, OCI_DTYPE_INTERVAL_DS);
    if (rc == OCI_ERROR)
      return rc;
  }
