/* Copyright (c) 1993, 2021, Oracle and/or its affiliates. */
/* All rights reserved.*/
...
/*
  ...
  MODIFIED
    jmmedina   01/12/21 - Add IBVC API
...
sword OCICollGetKeytype(OCIEnv *env, OCIError *err, const OCIColl *coll,
                       OCIKeyType *result );
/*
Name: OCICollGetKeytype - Get keytype that this ASCA is using
Parameters:
        env(IN) - pointer to OCI environment handle
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
                ...
                OCIErrorGet().
        coll (IN) - collection item.
        result (OUT) - Currently only OCI_KEYTYPE_STRING is supported
Description:
        Returns keytype that OCIColl IBVC is using.
Returns:
        OCI_SUCCESS if the function completes successfully.
        OCI_INVALID_HANDLE if 'env' or 'err' is NULL, or coll is not an IBVC.
*/

/*--------------------------- OCICollKeyAssignElem --------------------------*/

sword OCICollKeyAssignElem(OCIEnv *env, OCIError *err, const void *key,
                           ub4 keylen, const void *elem,
                           const void *elemind, OCIColl *coll);
/*
Name: OCICollKeyAssignElem -
...
Parameters:
        ...
        err (IN/OUT) - Error handle. If there is an error, it is
                ...
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        key (IN) - Pointer to the object that will be used as a key,
                   currently only oratext is supported.
        keylen(IN) - Length of the key provided
        elem (IN) - Element which is assigned from (source element)
        elemind (IN) [optional] - Pointer to the element's null indicator 
                information; if (elemind == NULL) then the null indicator
                information of the assigned element will be set to non-null.
        coll (IN) - Collection item.
Description:
        Assigns the given element to the given key in the related collection,
        if key already exists in collection, update the associated value
*/

/*--------------------------- OCICollKeyGetElem -----------------------------*/

sword OCICollKeyGetElem(OCIEnv *env, OCIError *err, const OCIColl *coll,
                        const void *key, ub4 keylen,
                        void **elem, void **elemind);
/*
Name: OCICollKeyGetElem -
...
Parameters:
        ...
        elemind (OUT) [optional] - Address of the null indicator information 
                ...
                information will NOT be returned
Description:
        Stores the address of the element associated to the given key in the
        location provided
*/
...
/*-------------------------- OCIIterKeyGetCurrent ---------------------------*/

sword OCIIterKeyGetCurrent(OCIEnv *env, OCIError *err, const OCIIter *itr,
                           const void **key, ub4 *keylen);
/*
  NAME: OCIIterKeyGetCurrent - OCIColl Iterator, get current element's key
  ...
  Parameters:
        ...
        err (IN/OUT) - Error handle. If there is an error, it is
                ...
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        itr (IN) - Iterator associated to an ASCA.
        key (OUT) - Reference to the key of the current element, should not
                    be modified.
        keylen(OUT) - OUT: size of the returned string.

        Description:
        Stores a copy of the current element's key in the given buffer, if
        buffer is of insufficient size, keylen will equal 0.
*/

/*----------------------------- OCIIterBatch --------------------------------*/

sword OCIIterBatch( OCIEnv *env, OCIError *err,
                    const OCIIter *itr, uword *nelems,
                    const void **keys, ub4 *lengths, void **elems,
                    boolean *eoc);
/*
  NAME: OCIIterBatch - OCIColl Iterator, get keys and elements in batch
  ...
  Parameters:
        ...
        err (IN/OUT) - Error handle. If there is an error, it is
                ...
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        itr (IN/OUT) - Iterator is updated to point to the (nelems)-th next
                       element
        nelems (IN/OUT) - IN: number of elements to retrieve
                          OUT: number of elements retrieved
        keys (OUT) - Pointer to a buffer of sufficient size to hold the
                     pointers to the keys of the next (nelems) elements,
                     keys should not be modified
        lengths (OUT) - Pointer to a buffer holding the lengths of the
                        extracted keys
        elems (OUT) - Pointer to a buffer of sufficient size to hold the
                      pointers to the next (nelems) elements
        eoc (OUT) - TRUE if after exiting, iterator is pointing to the
                    end of the collection
Description:
        Equivalent to calling OCIIterNext multiple times, stores all elements
        in provided buffers, if collection ends before fetching (nelems),
        eoc flag will be set and the number of elements copied will be
        copied into (nelems)
*/

/*---------------------------------- OCIIterReset ---------------------------*/

sword OCIIterReset(OCIEnv *env, OCIError *err, const OCIIter *itr);
...
/* OCITableKeyDelete */
sword OCITableKeyDelete(OCIEnv *env, OCIError *err, const void *key,
    ub4 keylen, OCITable *coll);

/* Name: OCITableKeyDelete
   Parameters:
        env(IN) - Pointer to OCI environment handle
        err (IN/OUT) - Error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        key (IN) - Key of the element that will be deleted.
        keylen(IN) - Length of the key provided
        coll (IN) - Collection item.
Description:
        Deletes the element associated to the given key. If the key does not
        exist in the given collection, will fail with ORA-21560
*/

/*------------------------------ OCITableKeyExists --------------------------*/

/* OCITableKeyExists */
sword OCITableKeyExists(OCIEnv *env, OCIError *err, const void *key,
    ub4 keylen, const OCITable *coll, boolean *exists);

/* Name: OCITableKeyExists
   Parameters:
        env(IN) - Pointer to OCI environment handle
        err (IN/OUT) - Error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        key (IN) - Key that will be searched in the collection
        keylen(IN) - Length of the key provided
        coll (IN) - Collection item.
        exists (OUT) - TRUE if key exists, FALSE otherwise
Description:
        Searches the collection for the given key
*/

/*------------------------------ OCITableKeyFirst ---------------------------*/

/* OCITableKeyFirst  */
sword OCITableKeyFirst(OCIEnv *env, OCIError *err, const void **key,
    ub4 *keylen, const OCITable *coll);

/*
  Name: OCITableKeyFirst
  Parameters:
        env(IN) - Pointer to OCI environment handle
        err (IN/OUT) - Error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
        ...
        key (OUT) - Reference to the key of the current element, should not
                    be modified.
        keylen(OUT) - Size of the returned string.
        coll (IN) - Collection item.
Description:
        Copies first, (minimum or smallest) key of the collection into
        the provided buffer
*/
...
/* OCITableKeyLast */
sword OCITableKeyLast(OCIEnv *env, OCIError *err, const void **key,
    ub4 *keylen, const OCITable *coll);

/*
  Name: OCITableKeyLast
  Parameters:
        env(IN) - Pointer to OCI environment handle
        err (IN/OUT) - Error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
        ...
        coll (IN) - Collection item.
Description:
        Copies last, (maximum or largest) key of the collection into
        the provided buffer
*/
...
/* OCITableKeyNext */
sword OCITableKeyNext(OCIEnv *env, OCIError *err, const void *key,
    ub4 keylen, const OCITable *coll, const void **next_key,
    ub4 *next_keylen, boolean *exists);
/*
  Name: OCITableKeyNext
  Parameters:
        env(IN) - Pointer to OCI environment handle
        err (IN/OUT) - Error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
                The error recorded in 'err' can be retrieved by calling
                OCIErrorGet().
        key (IN) - Reference key, following the collection's ordering, the
                   key immediately after this one will be searched for
        keylen(IN) - Length of the reference key
        coll (IN) - collection item.
        next_key (OUT) - Reference to the key of the current element,
                         should not be modified.
        next_keylen(OUT) - Size of the returned string.
        exists(OUT) - TRUE if a next key exists, FALSE otherwise
Description:
        Copies the key next to the reference key, following the collection's
        ordering
*/
...
/* OCITableKeyPrev */
sword OCITableKeyPrev( OCIEnv *env, OCIError *err, const void *key,
    ub4 keylen, const OCITable *coll, const void **prev_key,
    ub4 *prev_keylen, boolean *exists);
/*
  Name: OCITableKeyPrev
  Parameters:
        env(IN) - Pointer to OCI environment handle
        err (IN/OUT) - error handle. If there is an error, it is
                recorded in 'err' and this function returns OCI_ERROR.
                ...
                OCIErrorGet().
        key (IN) - Reference key, following the collection's ordering, the
                   key immediately before this one will be searched for
        keylen(IN) - Length of the reference key
        coll (IN) - Collection item.
        prev_key (OUT) - Reference to the key of the current element,
                         should not be modified.
        prev_keylen(OUT) - Size of the returned string.
        exists(OUT) - TRUE if a prev key exists, FALSE otherwise
Description:
        Copies the key previous to the reference key,
        following the collection's ordering
*/
